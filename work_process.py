...
from model.ResNet import TSEncoder
from model.Encoder import Encoder
from model.Mamba import TSCMamba
from model.mmm4tsc import TSMultiModalMamba
...
def work_process(args):
    ...
    if args.model == 'MLP':
        ...
        model = TSCMamba(input_size=UCR_dataset_train.input_size, output_size=UCR_dataset_train.output_size)
...
def vision_work_process(args):
    ...
    # 数据处理
    UCR_dataset_train = IMG_UCR_data_provider(args=args, dataset_type='train', transform=img_transform['train'])
    UCR_dataset_test = IMG_UCR_data_provider(args=args, dataset_type='test', transform=img_transform['test'])
    ...
    model = unireplknet_a(
        num_classes=UCR_dataset_train.output_size,
        in_chans=1,
        in_1k_pretrained=False
    )
    ...
    #     dim=224,
    #     n_layers=8,
    #     n_classes=UCR_dataset_test.output_size,
    #     block_type="bi_add_ff",
    #     pos_emb=True,
    # )
